...
install() {
  ...
  if [[ -e "${INSTALLDIR}" ]]; then
    ...
    chmod 755 "${INSTALLDIR}/runner.sh" 2>/dev/null
    sudo ln -sf "${INSTALLDIR}/runner.sh" "/usr/local/bin/runner.sh"
  fi
}
...
main() {
  ...
  if [[ -f "${INSTALLDIR}/runner.sh" ]]; then
    echo 'done!'
  else
    echo >&2 "Failed, ${INSTALLDIR}/runner.sh was not found"
    exit 1
  fi
}
